...
from views import Index, About, NotFound404, Contacts, Developments, Learn, CoursesList, CreateCourse, CreateCategory, \
...
def time_city_front(request):
    ...
    hour = int(time.localtime().tm_hour) + int(key_b)
    if hour > 24:
        hour = hour - 24
    request["city"] = key_a
    request['time_city'] = str(
        f"{hour}:{time.localtime().tm_min}:{time.localtime().tm_sec}")
